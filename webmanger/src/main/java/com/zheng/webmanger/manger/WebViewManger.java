...
public class WebViewManger {
    ...
    private CusWebViewClient cusWebViewClient;
    //发送消息后的回调数据
     ...
     * 设置加载效果和加载失败界面
     */
    public void setLoadingLayout(LoadView loadView){
        if(loadView != null && cusWebViewClient != null){
            cusWebViewClient.setLoadLayout(loadView);
        }
    }
